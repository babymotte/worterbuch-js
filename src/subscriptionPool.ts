import { Connection, Key, StateEvent, TransactionID, Value } from ".";
import { v4 as uuidv4 } from "uuid";

export type SubscriptionID = string;
export type Callback = (value: Value) => void;
...
export function subscriptionPool(wb: Connection): SubscriptionPool {
  ...
  const subscribe = (key: Key, callback: Callback) => {
    ...
    if (existingTransactionId !== undefined) {
      ...
      let transactionId = wb.subscribe(key, (event: StateEvent) => {
        if (event.value) {
          cache.set(key, event.value);
        } else {
          cache.delete(key);
        }
        subscriptions.get(transactionId)?.forEach((cb) => cb(event.value));
      });
